...
def compute_nepal_physics_loss(pinn, parser, x, y, t, tau_y, K, melt_rate, epoch):
    ...
    return (loss_p, h_w, v_max_val, torch.mean(expected_melted_vol).item(), Re_val, He_val, 
            ...
            rho_mix, water_fraction, u_water, v_water, friction_heat) # <-- Дописали тензоры сюда
        ...
        for epoch in range(8001):
            ...
            if epoch % 500 == 0 and epoch > 0:
                ...
                with torch.no_grad():
                    ...
                    print("   ------------------------------------------------------------")
                    with torch.no_grad():
                        # 🔥 ИСПРАВЛЕНИЕ: Выстраиваем тернарный оператор строго в ОДНУ строку!
                        avg_rho = torch.mean(rho_mix).item()
                        avg_water_frac = torch.mean(h_w_predicted / (h_w_predicted + 1e-5)).item() * 100.0
                        avg_plug_m = plug_zone_val if isinstance(plug_zone_val, float) else torch.mean(plug_zone_val).item()
                        
                        # Собираем реологические коэффициенты для датчиков
                        avg_v_water = torch.mean(torch.sqrt(u_water**2 + v_water**2 + 1e-5)).item()
                        avg_v_solid = torch.mean(torch.sqrt(u_solid**2 + v_solid**2 + 1e-5)).item()
                        slip_lead_percent = ((avg_v_water - avg_v_solid) / (avg_v_solid + 1e-5)) * 100.0
                    ...
                    # Динамический расчет мгновенного фронтального давления селя (кПа)
                    impact_pressure_kpa = (avg_rho * (v_max ** 2)) / 1000.0
                    ...
                    print(f"   ├── [Датчик 5: Саморазжижение]: Доля талой воды в теле потока = {water_fraction.mean().item()*100:.1f}%")
                    print(f"   ├── [Датчик 6: Пожирание русла]: Эрозия МакДугалла уносит {eroded_m3:.1f} м³ породы в секунду")
                    print(f"   ├── [Датчик 7: Структурная пробка]: Жесткое ядро комка занимает {plug_zone_val:.2f} м высоты вала")
